layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,table = {
    config: {
      checkName: 'LAY_CHECKED' //是否选中状态的字段�?
      ,indexName: 'LAY_TABLE_INDEX' //下标索引�?
    } //全局配置�?
    ,cache: {} //数据缓存
  ...
  ,thisTable = function(){
    ...
    if(id){
      ...
      thisTable.config[id] = options; //记录当前实例配置�?
    }
  ...
  Class.prototype.config = {
    ...
    ,cellMinWidth: 60 //所有单元格默认最小宽�?
    ,defaultToolbar: ['filter', 'exports', 'print'] //工具栏右侧图�?
      ...
      layui.each(item1, function(i2, item2){
        ...
        if(item2.colGroup || item2.colspan > 1){
          ...
          layui.each(options.cols[i1 + 1], function(i22, item22){
            //如果子列已经被标注为{HAS_PARENT}，或者子列累�? colspan 数等于父列定义的 colspan，则跳出当前子列循环
  ...
  Class.prototype.setColsWidth = function(){
    ...
    var getAutoWidth = function(back){
      ...
      layui.each(options.cols, function(i1, item1){
        layui.each(item1, function(i2, item2){
          var width = 0
          ,minWidth = item2.minWidth || options.cellMinWidth; //最小宽�?
    ...
    getAutoWidth();
    getAutoWidth(true); //重新检测分配的宽度是否低于最小列�?
  ...
  Class.prototype.pullData = function(curr){
    ...
    that.startTime = new Date().getTime(); //渲染开始时�?
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          if(res[response.statusName] != response.statusCode){
            ...
            options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请�?+视图渲染�?
          }
          that.setColsWidth();
    ...
    ,render = function(){ //后续性能提升的重�?
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          //td内容
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属�?
            var attr = [];
            if(item3.edit) attr.push('data-edit="'+ item3.edit +'"'); //是否允许单元格编�?
            if(item3.align) attr.push('align="'+ item3.align +'"'); //对齐方式
            if(item3.templet) attr.push('data-content="'+ content +'"'); //自定义模�?
            if(item3.toolbar) attr.push('data-off="true"'); //行工具列关闭单元格事�?
            if(item3.event) attr.push('lay-event="'+ item3.event +'"'); //自定义事�?
            if(item3.style) attr.push('style="'+ item3.style +'"'); //自定义样�?
            if(item3.minWidth) attr.push('data-minwidth="'+ item3.minWidth +'"'); //单元格最小宽�?
            return attr.join(' ');
          }() +' class="'+ function(){ //追加样式
            var classNames = [];
            if(item3.hide) classNames.push(HIDE); //插入隐藏列样�?
            if(!item3.field) classNames.push('layui-table-col-special'); //插入特殊列样�?
            return classNames.join(' ');
          }() +'">'
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标�?
    ...
    if(options.page){
      options.page = $.extend({
        ...
        ,jump: function(obj, first){
          if(!first){
            //分页本身并非需要做以下更新，下面参数的同步，主要是因为其它处理统一用到了它�?
  ...
  Class.prototype.sort = function(th, type, pull, formEvent){
    ...
    try {
      ...
      that.layHeader.find('th').find(ELEM_SORT).removeAttr('lay-sort'); //清除其它标题排序状�?
  ...
  Class.prototype.scrollPatch = function(){
    ...
    ,scollWidth = that.layMain.width() - that.layMain.prop('clientWidth') //纵向滚动条宽�?
    ,scollHeight = that.layMain.height() - that.layMain.prop('clientHeight') //横向滚动条高�?
  ...
  Class.prototype.events = function(){
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      switch(events){
        ...
        case 'LAYTABLE_EXPORT': //导出
          if(device.ie){
            layer.tips('导出功能不支�? IE，请�? Chrome 等高级浏览器导出', this, {
              tips: 3
            })
          } else {
            openPanel({
        ...
        case 'LAYTABLE_PRINT': //打印
          ...
          html.append(that.layTotal.find('table').html()) //输出合计�?
          
          html.find('th.layui-table-patch').remove(); //移除补丁
          html.find('.layui-table-col-special').remove(); //移除特殊�?
    ...
    var commonMember = function(sets){
      ...
      return $.extend({
        tr: tr //行元�?
        ,data: table.clearCacheKey(data) //当前行数�?
        ,del: function(){ //删除行数�?
        ...
        // 这里就是我们实际调用更新的代码位置�?
        ,update: function(fields){ //修改行数�?
          fields = fields || {};
      
          // 遍历传进来的每一个键值对（实际上就是我们的每一条数据）�?
          layui.each(fields, function(key, value){
            // data 是通过服务器返回的实际的每一行数�?
            if(key in data){
          ...
          that.eachCols(function (index, colOption) {
              ...
              if (colOption.toolbar) {
                  ...
                  // 拿到了之后，除了需要模板id外，还需要一个key。这个key在初始化时内部已经生�?
    ...
    //复选框选择
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){ //替代元素�? click 事件
    ...
    //行事�?
    that.layBody.on('mouseenter', 'tr', function(){ //鼠标移入�?
      var othis = $(this)
      ,index = othis.index();
      if(othis.data('off')) return; //不触发事�?
      that.layBody.find('tr:eq('+ index +')').addClass(ELEM_HOVER)
    }).on('mouseleave', 'tr', function(){ //鼠标移出�?
      var othis = $(this)
      ,index = othis.index();
      if(othis.data('off')) return; //不触发事�?
      that.layBody.find('tr:eq('+ index +')').removeClass(ELEM_HOVER)
    }).on('click', 'tr', function(){ //单击�?
      setRowEvent.call(this, 'row');
    }).on('dblclick', 'tr', function(){ //双击�?
  ...
  table.init = function(filter, settings){
    ...
    elemTable.each(function(){
      ...
      var cols = [], options = $.extend({
        ...
        ,even: typeof othis.attr('lay-even') === 'string' //偶数行背�?
      ...
      layui.each(item1, function(i2, item2){
        ...
        if(item2.colGroup){
          ...
          layui.each(cols[i1 + 1], function(i22, item22){
            //如果子列已经被标注为{PARENT_COL_INDEX}，或者子列累�? colspan 数等于父列定义的 colspan，则跳出当前子列循环
        ...
        if(item2.PARENT_COL_INDEX) return; //如果是子列，则不进行追加，因为已经存储在父列�?
        arrs.push(item2)
      });
    });
  ...
  table.checkStatus = function(id){
    ...
    return {
      data: arr //选中的数�?
      ,isAll: data.length ? (nums === (data.length - invalidNum)) : false //是否全�?
    };
  };
  ...
  table.exportFile = function(id, data, type){
    ...
    alink.href = 'data:'+ textType +';charset=utf-8,\ufeff'+ encodeURIComponent(function(){
      ...
      layui.each(data, function(i1, item1){
        var vals = [];
        if(typeof id === 'object'){ //如果 id 参数直接为表头数�?
  ...
  table.resize = function(id){
    //如果指定表格唯一 id，则只执行该 id 对应的表格实�?
    if(id){
      var config = getThisTableConfig(id); //获取当前实例配置�?
      if(!config) return;
